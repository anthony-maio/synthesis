...
from synthesis.sandbox.runtime import SandboxRuntime
...
class TDDSynthesizer:
    ...
    async def synthesize(
        ...
        try:
            ...
            for iteration in range(self.max_iterations):
                ...
                # Execute tests
                results, validation_result = await self._run_tests(code, test_suite)
                attempt.test_results = results
                attempt.tests_passed = sum(1 for r in results if r.passed)
                attempt.total_tests = len(results)

                # Check if all tests pass
                if attempt.tests_passed == attempt.total_tests and validation_result.is_valid:
                    attempt.status = SynthesisStatus.ENHANCING

                    # Enhance code with docstrings and type hints
                    enhanced_code = await self._enhance_implementation(code, requirement)
                    attempt.generated_code = enhanced_code
    ...
    async def _enhance_implementation(self, code: str, requirement: str) -> str:
        """Adds docstrings and type hints to the code."""
        prompt = f"""You are a senior Python developer focused on code quality.
Given the following Python code and its original requirement, please enhance it by adding:
1. A comprehensive Google-style docstring to the main function.
2. Strict type hints for all function arguments and return values.

The code should remain functionally identical. Only add documentation and type hints.
...
```"""
        ...
        if "```python" in enhanced_code:
            enhanced_code = enhanced_code.split("```python")[1].split("```")[0]
        elif "```" in enhanced_code:
            enhanced_code = enhanced_code.split("```")[1].split("```")[0]
...
Code:"""
    ...
    def _find_function_name(self, code: str) -> Optional[str]:
        """Parse code to find the first function definition"""
        try:
            tree = ast.parse(code)
    ...
    async def _run_tests(
        ...
        """
        Execute tests against implementation in a sandbox.

        Returns:
            A tuple containing the list of test results and the validation result.
        """
        ...
        if not validation_result.is_valid:
            error_message = f"Code validation failed: {[i.message for i in validation_result.issues]}"
        ...
        # 2. Find the function to test
        function_name = self._find_function_name(code)
        if not function_name:
            error_message = "No function definition found in generated code."
            for test_case in test_suite.tests:
                results.append(
                    TestResult(
                        test_case=test_case,
        ...
        # 3. Run each test in the sandbox
        for test_case in test_suite.tests:
            ...
            try:
                ...
                if exec_result.status == ExecutionStatus.SUCCESS:
                    actual_output = exec_result.output["result"]
                ...
                # Catch errors during the sandbox execution setup itself
                results.append(
                    TestResult(
                        test_case=test_case,
                        passed=False,
    ...
    async def _refine_implementation(
        ...
        """Generate refined implementation based on test failures and validation issues."""
        ...
        if validation_issues:
            for issue in validation_issues:
                static_analysis.append(
                    f"- {issue.severity.upper()}: {issue.message} (at {issue.location})"
                )

        prompt = f"""You are debugging and improving Python code. The implementation failed some tests or has quality issues.
...
{chr(10).join(static_analysis)}
"""

        prompt += """
Analyze the failures and static analysis issues. Generate an IMPROVED implementation that fixes these problems.
...
4. Logic errors
5. Security vulnerabilities (e.g., forbidden imports, dangerous functions)
...
```python"""
    ...
    async def _create_capability(
        ...
        name = "_".join(words[:3]) if len(words) >= 3 else "_".join(words)
